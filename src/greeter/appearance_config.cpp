...
bool parsePaletteColor(const nlohmann::json &palette, std::string_view key,
                       Color &out) {
...
std::filesystem::path greeterAppearanceConfigPath() {
  return greeter::appearance::manifestPath();
}
...
std::optional<GreeterSyncedAppearance> loadGreeterSyncedAppearance() {
  ...
  try {
    ...
    if (version != greeter::appearance::kManifestVersion) {
      kLog.warn("unsupported synced appearance version {} in '{}'", version,
                path.string());
      return std::nullopt;
    }
    ...
    if (!parsePaletteColor(paletteJson, "primary",
                           appearance.palette.primary) ||
        !parsePaletteColor(paletteJson, "on_primary",
                           appearance.palette.onPrimary) ||
        !parsePaletteColor(paletteJson, "secondary",
                           appearance.palette.secondary) ||
        !parsePaletteColor(paletteJson, "on_secondary",
                           appearance.palette.onSecondary) ||
        !parsePaletteColor(paletteJson, "tertiary",
                           appearance.palette.tertiary) ||
        !parsePaletteColor(paletteJson, "on_tertiary",
                           appearance.palette.onTertiary) ||
        !parsePaletteColor(paletteJson, "error", appearance.palette.error) ||
        !parsePaletteColor(paletteJson, "on_error",
                           appearance.palette.onError) ||
        !parsePaletteColor(paletteJson, "surface",
                           appearance.palette.surface) ||
        !parsePaletteColor(paletteJson, "on_surface",
                           appearance.palette.onSurface) ||
        !parsePaletteColor(paletteJson, "surface_variant",
                           appearance.palette.surfaceVariant) ||
        !parsePaletteColor(paletteJson, "on_surface_variant",
                           appearance.palette.onSurfaceVariant) ||
        !parsePaletteColor(paletteJson, "outline",
        ...
        !parsePaletteColor(paletteJson, "on_hover",
                           appearance.palette.onHover)) {
    ...
    if (wallpaperIt != root.end() && wallpaperIt->is_object()) {
      ...
      if (const auto pathValue = wallpaper.find("path");
          pathValue != wallpaper.end() && pathValue->is_string()) {
        appearance.wallpaperPath = pathValue->get<std::string>();
      }
      if (const auto fillMode = wallpaper.find("fill_mode");
          fillMode != wallpaper.end() && fillMode->is_string()) {
        if (const auto parsed = greeter::appearance::parseFillMode(
                fillMode->get<std::string>())) {
          appearance.wallpaperFillMode = *parsed;
        }
      }
      if (const auto fillColor = wallpaper.find("fill_color");
          fillColor != wallpaper.end() && fillColor->is_string()) {
        Color color;
    ...
    kLog.warn("failed to parse synced appearance '{}': {}", path.string(),
              e.what());
    return std::nullopt;
  }
}
